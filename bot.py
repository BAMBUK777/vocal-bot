...
if not TOKEN or not ADMIN_IDS:
    raise RuntimeError("Не заданы обязательные переменные окружения TOKEN и ADMIN_IDS")
...
def init_db():
    ...
    """)
    # Миграции: по необходимости дополняем колонками
...
def send_date_selection(message):
    ...
    for d in range(14):
        ...
        if 1 <= day.weekday() <= 4:  # вторник–пятница
...
def my_appointments(message):
    ...
    for appt_id, teacher, date_iso, time_slot in rows:
        ...
        kb = types.InlineKeyboardMarkup(row_width=2)
        kb.add(
            types.InlineKeyboardButton('❌ Отменить запись', callback_data=f"cancel_request:{appt_id}"),
            types.InlineKeyboardButton('↩️ Назад', callback_data='back')
        )
